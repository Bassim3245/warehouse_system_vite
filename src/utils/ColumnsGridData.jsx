...
export const inventoryColumnGrid = ({ t, renderMenuItem, openMovement }) => [
  { field: "id", headerName: "ID", hideable: false },
...
export const entityUserColumnGrid = ({
  ...
  },
  {
    ...
    renderCell: (params) => {
      return (
        <p>
          {params?.row?.warehouse_type === "main"
            ? "المخزن الرئيسي"
            : params?.row?.warehouse_type === "production"
              ? "المخزن الانتاج التام"
              : "----"}
        </p>
      );
    },
...
export const inventoryImportManagement = ({
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        ...
        >
          {params?.row?.quantity
            ? FormatDataNumber(params.row.quantity)
            : "---"}
        </div>
      ),
    },
      ...
      renderCell: (params) => (
        ...
        >
          {params?.row?.remaining_quantity
            ? FormatDataNumber(params.row.remaining_quantity)
            : "---"}
        </div>
      ),
    },
  ...
  // 🔹 Conditional date column
  if (searchParams?.get("documentType") === "in") {
    columns.push({
      field: "purchase_date",
      headerName: t("تاريخ الشراء"),
      flex: 1,
    ...
    renderCell: (params) => (
      <div style={{ display: "flex", justifyContent: "center", width: "100%" }}>
        {params?.row?.total_quantity
          ? FormatDataNumber(params?.row?.total_quantity)
          : "---"}
      </div>
    ),
  },
...
export const documentArchiveMonthlyGrid = ({
  t,
  openMovement,
  documentLabel,
  documentType,
