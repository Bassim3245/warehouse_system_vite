...
const MonthlyLocks = () => {
  ...
  const { unlockMonth } = useMonthlyClose();
  ...
  const handleUnlockMonth = async (lockId) => {
    await unlockMonth(lockId, fetchInformation);
  };
  ...
  const filteredLocks =
    lockData?.filter((lock) => {
      if (selectedWarehouse && lock.warehouse_id !== selectedWarehouse)
        return false;
      return true;
    }) || [];
      ...
      {isAlart ? (
        ...
        >
          <AlertTitle sx={{ fontWeight: 700, fontSize: "1.1rem" }}>
            تنويه: الميزة قيد التحديث
          </AlertTitle>
          نحن نعمل حالياً على تطوير وتحسين نظام الأرشفة وإغلاق الأشهر (Live)
          لضمان أعلى مستويات الدقة والموثوقية. قد تلاحظ بعض التغييرات أثناء
          العمل.
        </Alert>
      ) : (
        <Box>
          ...
          {/* Header */}
          <Header title="إغلاق الأشهر " dir="rtl" />
                ...
                <Grid size={{ xs: 12, sm: 6, md: 6, lg: 4 }} key={lockItem.id}>
                  <Card
                    sx={{
                      height: "100%",
                      borderRight: 4,
                      borderColor: lockItem?.is_locked
                        ? "error.main"
                        : "success.main",
                      transition: "all 0.3s",
                      "&:hover": {
